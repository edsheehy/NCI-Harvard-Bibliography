...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt"
				...
				<xsl:for-each select="b:Author/b:Interviewee/b:NameList/b:Person">
					...
					<xsl:choose>
						<xsl:when test="$cIntervieweeLastName=1">
							<xsl:value-of select="b:Last"/>
							<xsl:choose>
								<xsl:when test ="$cIntervieweeFirstName=0 and $cIntervieweeMiddleName=0">
									<xsl:choose>
										<xsl:when test ="(position()+1)=$cInterviewees">
											<xsl:value-of select="' and '"/>
										</xsl:when>
										<xsl:when test ="position()=$cInterviewees">
											<xsl:call-template name ="templ_prop_Space"/>
										</xsl:when>
										<xsl:otherwise>
											<xsl:call-template name ="templ_prop_ListSeparator"/>
							...
							<xsl:choose>
								<xsl:when test ="$cIntervieweeMiddleName=0">
									<xsl:choose>
										...
										<xsl:when test ="$cInterviewees>position()">
											<xsl:call-template name ="templ_prop_ListSeparator"/>
										</xsl:when>
										<xsl:otherwise>
											<xsl:call-template name ="templ_prop_Space"/>
				...
				<xsl:for-each select="b:Author/b:Author/b:NameList/b:Person">
					...
					<xsl:choose>
						<xsl:when test="$cAuthorLastName=1">
							<xsl:value-of select="b:Last"/>
							<xsl:choose>
								<xsl:when test ="$cAuthorFirstName=0 and $cAuthorMiddleName=0">
									<xsl:choose>
										<xsl:when test ="(position()+1)=$cAuthors">
											<xsl:value-of select="' and '"/>
										</xsl:when>
	...
	<xsl:template name="BibDisplayTitleWeb">
		<xsl:variable name="cTitle">
			<xsl:value-of select="count(b:Title)"/>
		</xsl:variable>
		<xsl:if test ="$cTitle!=0">
	...
	<xsl:template name="BibDisplaySiteName">
		<xsl:variable name="cSiteName">
			<xsl:value-of select="count(b:InternetSiteTitle)"/>
		</xsl:variable>
		<xsl:if test ="$cSiteName!=0">
			<xsl:call-template name="right-trim">
			...
			<xsl:choose>
				...
				<xsl:when test="$SourceType = 'InternetSite'">
					<xsl:element name="p">
						...
						<xsl:call-template name = "BibDisplayAuthor"/>
						<xsl:call-template name = "BibDisplayYear"/>
						<xsl:call-template name ="BibDisplayTitleWeb"/>
						<xsl:call-template name ="BibDisplayStrOnline"/>
						<i>
							<xsl:call-template name ="BibDisplaySiteName"/>
						</i>
